...
public class Reduce
    ...
    public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
        ...
        org.robby.mr.pagerank.Node originalNode = new org.robby.mr.pagerank.Node();

        for (Text textValue : values) {
            System.out.println("  input -> V[" + textValue + "]");
            org.robby.mr.pagerank.Node node = Node.fromMR(textValue.toString());
            if (node.containsAdjacentNodes()) {
                //包含节点信息的是初始的数据，不做reduce处理
                // the original node
                //
                originalNode = node;
            } else {
